'''
Requis pour l'utilisaion du programme :
- Installer psutil --> pip install psutil
'''
...
'''
# Vérifie sur toutes les interfaces réseaux la liste de snics et retourne l'adresse IP de la carte réseau utilisée
def get_local_ip():
    for iface, snics in psutil.net_if_addrs().items():
        for snic in snics:
            if snic.family == socket.AF_INET and not snic.address.startswith("127."): # On ne garde que les IPv4 avec AF_INET et on évite le localhost
                return snic.address
'''
...
def udp_listener():
    ...
    while True: # Tant que la condition est vraie
        ...
        try:
            ...
            logs.append("[ERREUR] Message UDP mal formaté.")
    ...
    with conn: # Objet utilisé pour communiquer avec le pair
        try:
            ...
            if first_data.startswith(b"[KEY]"): # Si les données commencent par [KEY] alors
                try:
                    key, rest = first[5:].split(b"\n", 1)
                except ValueError:
                    key = first[5:]
                    rest = b''
                public_keys[addr[0]] = key.decode()
                if rest:
                    received_messages.append((addr, rest.decode(errors='replace')))
                '''
                key = first_data.decode()[5:] # On évite le préfixe [KEY] et on enregistre la clé
                public_keys[addr] = key # On enregistre la clé dans le dictionnaire
                logs.append(f"[CLÉ PUBLIQUE] Reçue de {addr} : {key}") # Logs
                '''
            else:
                print()
            ...
            while True: # Tant que c'est vrai
                ...
                if not data: # Un failsafe
                    break
                msg = data.decode(errors="replace").strip() # On enregistre la donnée dans message
                print(f"[DEBUG] Message brut reçu : {repr(msg)}") # Petit print de debug
    ...
    while True: # Affichage du menu tant qu'on quite pas
        ...
        if choix == "1":
            ...
            try:
                ...
                logs.append("[DEBUG] - Contenu du message : " + msg)
                with socket.create_connection(peer, timeout=5) as sock:
                    random_digits = random.randint(100, 999)
                    fake_key = f"FAKE_PUBLIC_KEY_{random_digits}" # Création d'une fake clé publique pour le test avec des chiffres aléatoires
                    sock.sendall(f"[KEY]{fake_key}\n".encode()) # Envoi aux pairs
                    logs.append("[DEBUG] - Passage du test d'envoi")
            ...
            for addr, msg in received_messages:
                print(f"De {addr} > {repr(msg)}") # Affichage des messages reçus
        elif choix == "4":
            print("\n--- Logs ---")
            for log in logs[-20:]:
...
'''
def tcp_client(peer):
    ip, port = peer # On met les infos dans des variables
    try:
        with socket.create_connection((ip, port), timeout=5) as sock: # On crée une connexion a un pair
            print(f"[TCP] Connecté à {peer}")
            while True: # Tant que c'est vrai on peut envoyer un message via le réseau
                msg = input(f"[à {peer}] > ")
                sock.sendall(msg.encode())
    except Exception as e:
