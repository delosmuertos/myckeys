...
# Fonction s'occupant de récupérer la carte réseau en cours d'utilisation ainsi que son adresse IP
def get_local_ip():
    try:
...
def handle_client(conn, addr):
    # Interprétation des échanges de clés avec le préambule PUBKEY
    try:
        ...
        # Interprétation des échanges entre les groupes avec GROUPMSG
            ...
            return
        # Interprétation des requêtes pour rejoindre un groupe avec le préambule JOINGROUP
...
def creer_groupe(nom, membres):
    ...
    for ip in membres:
        if ip == my_ip:
            logs.append(f"[DEBUG] Je suis {ip}, je ne m’envoie pas JOINGROUP, Duh")
            continue
